...
namespace X2295
{
	...
	{
		DrawLine(ImVec2(x, y), ImVec2(x + w, y), color, 1.0f); // top 
		DrawLine(ImVec2(x, y - 1.3f), ImVec2(x, y + h + 1.4f), color, 1.0f); // left
		DrawLine(ImVec2(x + w, y - 1.3f), ImVec2(x + w, y + h + 1.0f), color, 1.0f);  // right
		DrawLine(ImVec2(x, y + h), ImVec2(x + w, y + h), color, 1.0f);   // bottom 
	}
